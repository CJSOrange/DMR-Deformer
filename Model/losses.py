...
def ncc_loss(I, J, win=None):
    '''
    The input size: [B,C,D,W,H]
    and the convolution is used to realize the specified window sum when calculating NCC
    '''
    ndims = len(list(I.size())) - 2
    assert ndims in [1, 2, 3], "volumes should be 1 to 3 dimensions. found: %d" % ndims
    if win is None:
        win = [9] * ndims
    sum_filt = torch.ones([1, 1, *win]).cuda(args.local_rank, non_blocking=True)
...
def cc_loss(x, y):
    # Calculate according to cross correlation formula
    dim = [2, 3, 4]
...
if __name__ == "__main__":
    ...
    a = torch.rand(1, 160, 192, 160, 3)
    b = Get_Ja(a)
    d = b.size(1) * b.size(2) * b.size(3)
    c = (b < 0).nonzero()
    print(c.size(0) / d * 100)
